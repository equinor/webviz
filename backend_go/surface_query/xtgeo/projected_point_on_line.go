...
func ProjectedPointOnLine(x1, y1, z1, x2, y2, z2, x3, y3, z3 float64) (x, y, z, rel float64, errCode int) {
	...
	// Length of segment
	dlen := math.Sqrt(float64(math.Pow(float64(x2-x1), 2) + math.Pow(float64(y2-y1), 2) + math.Pow(float64(z2-z1), 2)))
	if dlen < floatEps {
		return 0, 0, 0, 0, 3
	}

	// Find u
	u := (((x3 - x1) * (x2 - x1)) + ((y3 - y1) * (y2 - y1)) + ((z3 - z1) * (z2 - z1))) / math.Pow(float64(dlen), 2)
	...
	rellen := math.Sqrt(float64(math.Pow(float64(x1-x), 2) + math.Pow(float64(y1-y), 2) + math.Pow(float64(z1-z), 2)))
	fullen := math.Sqrt(float64(math.Pow(float64(x1-x2), 2) + math.Pow(float64(y1-y2), 2) + math.Pow(float64(z1-z2), 2)))
