...
# A lot of the configuration will be read from  environment variables.
# Notable environment variables that are consumed are:
# - APPLICATIONINSIGHTS_CONNECTION_STRING
# - OTEL_SERVICE_NAME
# - OTEL_RESOURCE_ATTRIBUTES
def setup_azure_monitor_telemetry(fastapi_app: FastAPI):
    # Under ideal circumstances this should be the only call needed to configure the entire
    # telemetry stack. However, it seems that there are multiple glitches.
    ...
    #   specifically it seems it has to happen before any import of FastAPIP
    ...
    # The default log handler that is added by configure_azure_monitor() does not obey the specified log message
    # format string, even if it is set using OTEL_PYTHON_LOG_FORMAT. It justs logs the raw message string.
    # We therefore replace it with a custom handler that does formatting and set a format string that ensures
