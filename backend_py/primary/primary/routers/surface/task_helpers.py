...
async def determine_surf_task_fingerprint_async(
    ...
    return await determine_statistical_task_fingerprint_async(
...
async def determine_statistical_task_fingerprint_async(
    authenticated_user: AuthenticatedUser, case_uuid: str, ensemble_name: str, task_identity: str
) -> str:
    ...
    task_fp = sha256((task_identity + ensemble_fp).encode()).hexdigest()
...
async def submit_and_track_stat_surf_task_async(
    ...
    return await submit_and_track_statistical_task_async(
...
async def submit_and_track_statistical_task_async(
    submit_task: Callable[[], Awaitable[str]], task_tracker: TaskMetaTracker, task_fingerprint: str
) -> TaskMeta:
    task_start_time_utc_s = time.time()

    sumo_task_id = await submit_task()
