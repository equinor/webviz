...
class Grid3dAccess:
    ...
    async def get_properties_info_arr_async(
        ...
        if "TROLL" in grid3d_geometry_name:
            query = grid3d_properties_collection._add_filter(name=grid3d_geometry_name,iteration=self._iteration_name,realization=realization)
        ...
        async for property_meta in grid3d_properties_collection_filtered:
            ...
            property_name = property_meta["data"]["name"] if not "TROLL" in grid3d_geometry_name else property_meta["data"]["tagname"]
