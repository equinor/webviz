...
class Grid3dAccess:
    ...
    async def get_properties_info_arr_async(
        ...
        if grid3d_geometry_name == "TROLL_PRED_IX":
            query = grid3d_properties_collection._add_filter(name="TROLL_PRED_IX",iteration=self._iteration_name,realization=realization)
            grid3d_properties_collection_filtered= Grid3dPropertyCollection(grid3d_geometry_name, self._sumo_client, self._case_uuid, query, None)
        else:
        ...
        async for property_meta in grid3d_properties_collection_filtered:
            ...
            property_name = property_meta["data"]["name"] if grid3d_geometry_name != "TROLL_PRED_IX" else property_meta["data"]["tagname"]
