...
from .inplace_volumetrics_types import InplaceVolumetricsIdentifier, InplaceVolumetricsIdentifierWithValues, InplaceVolumetricsTableDefinition
...
class InplaceVolumetricDataEntry(BaseModel):
    ...
    identifier_values: List[Union[str, int]]
...
class InplaceVolumetricsAccess:
    ...
    async def get_table_definition(self, vol_table_name: str) -> InplaceVolumetricsTableDefinition:
        ...
        for col in vol_table_column_names:
            if col in IGNORED_COLUMN_NAMES:
                continue
            if col not in ALLOWED_IDENTIFIER_COLUMN_NAMES + ALLOWED_RESULT_COLUMN_NAMES:
        ...
        if invalid_column_names:
            ...
            #     Service.SUMO,
            # )
        identifier_names = [col for col in vol_table_column_names if col in ALLOWED_IDENTIFIER_COLUMN_NAMES]
        ...
        for identifier_column_name in identifier_names:
            ...
            if any([val in IGNORED_IDENTIFIER_COLUMN_VALUES for val in unique_values]):
                LOGGER.warning(
    ...
    async def get_aggregated_volumetric_table_data_async(
        ...
        identifiers_with_values: List[InplaceVolumetricsIdentifierWithValues] = None,
        aggregate_by_each_list: Sequence[InplaceVolumetricsIdentifier] = None,
        ...
        for index in identifiers_with_values:
    ...
    async def get_volumetric_data_async(
        ...
        index_columns = [col for col in arrow_table.column_names if col in ALLOWED_IDENTIFIER_COLUMN_NAMES]

        # Filter invalid data. Hopefully TMP
        for index_column in index_columns:
            arrow_table = _filter_arrow_table_by_exclusion(arrow_table, index_column, IGNORED_IDENTIFIER_COLUMN_VALUES)
        ...
        for i in range(num_entries):
            entry = InplaceVolumetricDataEntry(
                ...
                identifier_values=[data_dict.get(index_name)[i] for index_name in index_columns],
            )
