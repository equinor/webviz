...
class SnapshotAccess:
    ...
    async def create(cls, user_id: str) -> "SnapshotAccess":
        container = await ContainerAccess.create("persistence", "snapshots")
        return cls(user_id=user_id, container_access=container)

    async def get_snapshot_by_id(self, session_id: str) -> Optional[SessionRecord]:
    ...
    async def get_all_snapshot_metadata_for_user(self) -> List[SessionMetadataSummary]:
