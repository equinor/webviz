...
@pytest.mark.parametrize("special_value", ["-", "*", "TAGNAME", "STDRES"])
def test_no_component_value_has_special_meaning(special_value: str) -> None:
    addr0 = RealizationSurfaceAddress(
        "UUID123", "iter-0", "surf.name", TagNameAttribute(tag_name=special_value), 1, None
    )
...
def test_decode_surf_addr_str() -> None:
    ...
    stdres_no_sub_addr = decode_surf_addr_str("REAL~~UUID123~~iter-0~~surf.name~~STDRES~~structure_depth_surface~~~~-1")
