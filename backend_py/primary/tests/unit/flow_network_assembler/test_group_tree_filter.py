...
@pytest.fixture
def sample_group_tree_df() -> pl.DataFrame:
...
def test_gruptree_filtering_excludes_dates_without_gruptree(sample_group_tree_df: pl.DataFrame) -> None:
    """Test that GRUPTREE filtering only includes dates where GRUPTREE is defined."""
    model = GroupTreeDataframeModel(sample_group_tree_df)
...
def test_gruptree_filtering_excludes_branprop_rows(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_gruptree_filtering_includes_welspecs_with_valid_parent(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_branprop_filtering_includes_multiple_dates(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_branprop_filtering_excludes_gruptree_rows(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_branprop_filtering_includes_welspecs_per_date(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_model_extracts_wells(sample_group_tree_df: pl.DataFrame) -> None:
...
def test_model_detects_tree_types(sample_group_tree_df: pl.DataFrame) -> None:
    ...
    empty_df = pl.DataFrame(
        {
            ...
            "CHILD": pl.Series([], dtype=pl.String),
            "KEYWORD": pl.Series([], dtype=pl.String),
            "PARENT": pl.Series([], dtype=pl.String),
        }
    )
...
def test_missing_required_column_raises_error() -> None:
    ...
    incomplete_df = pl.DataFrame(
        {
            "DATE": [datetime.datetime(2032, 12, 1)],
            "CHILD": ["FIELD"],
            "KEYWORD": ["GRUPTREE"],
        }
    )
    ...
    with pytest.raises(
        ...
        GroupTreeDataframeModel(incomplete_df)
