...
export const Select = withDefaults<SelectProps>()(defaultProps, (props) => {
    ...
    const [selectionAnchor, setSelectionAnchor] = React.useState<number>(-1);
    ...
    const toggleValue = React.useCallback(
        function toggleValue(option: SelectOption, index: number) {
            ...
            if (props.multiple) {
                if (keysPressed.includes("Shift")) {
                    if (index !== currentIndex && selectionAnchor === -1) {
        ...
        [props.multiple, options, selected, onChange, keysPressed, selectionAnchor, setCurrentIndex, setSelected]
    );
    ...
    }, [currentIndex, selected, filteredOptions, props.size, hasFocus, startIndex, toggleValue, keysPressed]);
    ...
    return (
        <BaseComponent disabled={props.disabled}>
            ...
            <select multiple={props.multiple} className="w-full h-80">
                {options.map((item) => (
