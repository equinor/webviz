            ...
            if (plotType === PlotType.ParameterCorrelation) {
                ...
                for (let rowIndex = 0; rowIndex < numRows; rowIndex++) {
                    for (let colIndex = 0; colIndex < numCols; colIndex++) {
                        ...
                        const { trace, dimensions } = plotParallelCoordinates(
                            parameters,
                            responseData,
                            rankedParameters,
                            { colorscale: "RdBu", showColorScale: true }, // Optional styling
                        );
                        figure.addTrace(trace, rowIndex + 1, colIndex + 1);
                        const width = Math.max(wrapperDivSize.width, dimensions * 80 + 250);
                        figure.updateLayout({ width: width });
...
function plotParallelCoordinates(
    ...
    const parameterDataMap = new Map<string, Parameter>();
    allParameters.forEach((p) => parameterDataMap.set(p.name, p));

    const parameterDimensions: any = [];

    const orderedParameterNames = rankedParameters.map((p) => p.ident.name);

    orderedParameterNames.forEach((paramName) => {
        const parameterData = parameterDataMap.get(paramName);
        if (parameterData) {
