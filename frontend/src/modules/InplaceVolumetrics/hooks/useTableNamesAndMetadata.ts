...
export type TableNamesAndMetaData = {
    data: Array<{
        ...
        tableNamesAndMetadata?: InplaceVolumetricTableDefinition_api[];
    }>;
    isFetching: boolean;
};
    ...
    return useQueries({
        queries: ensembleIdents.map((ensembleIdent) => ({
            ...
            queryFn: () => {
                ...
                return apiService.inplaceVolumetrics.getTableDefinitions(caseUuid ?? "", ensembleName ?? "");
            },
            staleTime: STALE_TIME,
            gcTime: CACHE_TIME,
            enabled: Boolean(ensembleIdent),
        })),
        combine: (results: UseQueryResult<Array<InplaceVolumetricTableDefinition_api>>[]) => ({
