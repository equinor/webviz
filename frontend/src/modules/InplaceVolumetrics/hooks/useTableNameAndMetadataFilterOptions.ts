    ...
    if (!tableNamesAndMetadata.isFetching && !isEqual(prevTableNamesAndMetadata, tableNamesAndMetadata)) {
        ...
        for (const tableNamesAndMetadataItem of tableNamesAndMetadata.data) {
            ...
            for (const tableMetadata of tableNamesAndMetadataItem.tableNamesAndMetadata) {
                ...
                for (const categoricalColumnMetadata of tableMetadata.categories) {
                    if (!newReducedTableNamesAndMetadata.categories[categoricalColumnMetadata.category_name]) {
                        newReducedTableNamesAndMetadata.categories[categoricalColumnMetadata.category_name] = [];
                    }

                    for (const value of categoricalColumnMetadata.unique_values) {
                        if (
                            !newReducedTableNamesAndMetadata.categories[
                                categoricalColumnMetadata.category_name
                            ].includes(value)
                        ) {
                            newReducedTableNamesAndMetadata.categories[categoricalColumnMetadata.category_name].push(
                                value
                            );
                        }
                    }
                ...
                for (const numericalColumnName of tableMetadata.result_names) {
