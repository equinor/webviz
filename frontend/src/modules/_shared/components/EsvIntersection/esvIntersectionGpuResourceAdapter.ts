 ...
 *
 * Pixi rebuilds its own GPU resources (textures, buffers, ...) automatically once the browser
 * restores a lost context, so this is a thin wrapper around {@link createCanvasContextAdapter} that
 * just wires in the controller's canvas and a redraw nudge - recovery only needs the `"redraw"`
 * strategy, no remount.
 *
