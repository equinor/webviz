...
import {
    ...
    createPercentileMemberTooltipFormatter,
} from "./tooltips";
...
export function buildPercentileRangeSeries(
    ...
    const { showMemberPoints = false, yAxisPosition = 0, centerStatistic = "p50", showWhiskers = true } = options;
    ...
    if (showMemberPoints) {
        series.push(createMemberPointSeries(trace, yAxisPosition, axisIndex));
    }
...
function createMemberPointSeries(
    trace: DistributionTrace,
    yAxisPosition: number,
    axisIndex: number,
): ScatterSeriesOption {
    const tooltipFormatter = createPercentileMemberTooltipFormatter(trace.name, trace.color);
    ...
    return (
        {
            ...
            xAxisIndex: axisIndex,
            yAxisIndex: axisIndex,
            data: trace.values.map(function buildPercentileMemberPoint(value, index) {
                return {
