...
export function formatStatisticsAxisTooltip(params: CallbackDataParams | CallbackDataParams[]): string {
    ...
    const rows = Array.from(groupedRows.values()).map(function mapGroupedRowToTooltipRow(groupedRow) {
        return {
...
export function formatMemberTooltipContent(input: {
    ...
    const memberLabel = input.memberLabel ?? "Member";
...
function formatStatisticValuesInline(valuesByStatKey: Map<string, string>): string {
    const orderedEntries = Array.from(valuesByStatKey.entries()).sort(function compareStatOrder([left], [right]) {
