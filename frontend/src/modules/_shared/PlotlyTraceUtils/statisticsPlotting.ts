...
import { PlotDataWithLegendRank } from "./types";
...
export function createStatisticsTraces({
    ...
    type = "scatter",
}: CreateStatisticsTracesOptions): Partial<PlotDataWithLegendRank>[] {
    // NOTE:
    // - hovermode? not exposed?

    validateStatisticsData(data);

    function getDefaultTrace(statisticsName: string, values: number[]): Partial<PlotDataWithLegendRank> {
        const trace: Partial<PlotDataWithLegendRank> = {
    ...
    const traces: Partial<PlotDataWithLegendRank>[] = [];
