...
export function usePublishToDataChannels(
    ...
    { entries, responseName, dataChannelDepth, makeEnsembleDisplayName, isFetching }: PublishToDataChannelsParams,
): void {
    ...
    if (dataChannelDepth !== null && responseName) {
        ...
        for (const [key, group] of groups) {
            const displayName = `${responseName} @ ${dataChannelDepth.toFixed(1)} TVD (${makeEnsembleDisplayName(
                group.ensembleIdent,
            )})`;
            contents.push({
                contentIdString: key,
    ...
    viewContext.usePublishChannelContents({
        ...
        dependencies: [entries, responseName, dataChannelDepth],
