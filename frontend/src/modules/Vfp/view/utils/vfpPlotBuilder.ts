...
export class VfpPlotBuilder {
    private _vfpDataAccessor: VfpApiDataAccessor;
    private _colorScale: ColorScale;

    constructor(vfpDataAccessor: VfpApiDataAccessor, colorScale: ColorScale) {
            ...
            for (let j = 0; j < selectedWfrIndices.length; j++) {
                for (let k = 0; k < selectedGfrIndices.length; k++) {
                    for (let l = 0; l < selectedAlqIndices.length; l++) {
                        ...
                        const trace = this.getSingleVfpProdTrace(
                            thpIndex,
                            wfrIndex,
                            gfrIndex,
                            alqIndex,
