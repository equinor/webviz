...
test.describe("InplaceVolumesTable CSV download", () => {
    test("realization download contains all rows, including off-screen virtualized ones", async ({ mount, page }) => {
        ...
        const cmp = await mount(<InplaceVolumesTableHarness mode="realization" columnsConfig={columnsConfig} rows={rows} />);
    ...
    test("statistical download has a single flattened header row", async ({ mount, page }) => {
        ...
        const cmp = await mount(<InplaceVolumesTableHarness mode="statistical" columnsConfig={columnsConfig} rows={rows} />);
    ...
    test("filename matches the expected pattern", async ({ mount, page }) => {
        ...
        expect(download.suggestedFilename()).toMatch(/^InplaceVolumesTable_(Realizations|Statistics)_\d{8}_\d{6}\.csv$/);
    });
