 ...
 *     Playwright pick it up), and its `_story.template.meta.ts` sidecar to `<yourStory>.meta.ts`.
 ...
 *  4. Fill in the `meta` in the `<yourStory>.meta.ts` sidecar, then run `npm run generate:tutorials-manifest`.
...
import { meta } from "./_story.template.meta";
