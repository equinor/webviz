...
describe("computeTimeseriesStatistics", () => {
    it("returns empty arrays for no members", () => {
        const result = computeTimeseriesStatistics([]);
        expect(result.mean).toEqual([]);
        expect(result.p10).toEqual([]);
    });

    it("computes per-timestep statistics across members", () => {
        // 3 members, 2 timesteps
        const members = [
            [10, 20],
            [20, 40],
            [30, 60],
        ];
        const result = computeTimeseriesStatistics(members);
