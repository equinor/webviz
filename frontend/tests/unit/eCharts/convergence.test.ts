...
describe("calcConvergence", () => {
    ...
    it("first point mean equals its own value", () => {
        const result = calcConvergence([{ realization: 1, value: 10 }]);
        expect(result).toHaveLength(1);
        expect(result[0].mean).toBe(10);
        expect(result[0].realization).toBe(1);
    });

    it("running mean converges to the overall mean", () => {
        const pairs = [
            { realization: 1, value: 10 },
            { realization: 2, value: 20 },
            { realization: 3, value: 30 },
            { realization: 4, value: 40 },
        ];
    ...
    it("preserves realization IDs", () => {
        const pairs = [
            { realization: 5, value: 1 },
            { realization: 10, value: 2 },
        ];
        const result = calcConvergence(pairs);
        expect(result[0].realization).toBe(5);
        expect(result[1].realization).toBe(10);
    });
    ...
    it("p10 and p90 narrow as realizations grow", () => {
